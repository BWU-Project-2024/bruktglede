...
export async function fetchStoreInfo() {
    const supabase = createClient();
    const { data: storeinfo, error } = await supabase.from("Store").select("*");
...
export async function fetchStationsData(storeId) {
    const supabase = createClient();
    ...
    if (error) {
        throw new Error("Error fetching station data for store " + storeId + ": " + error.message);
    }

    return stationData || [];
}

export async function fetchTagsData(stationId) {
    const supabase = createClient();
    const { data: stationTagData, error: stationTagError } = await supabase
        .from("Station_Tags")
        .select("tag_id")
        .eq("station_id", stationId);

    if (stationTagError) {
        throw new Error("Error fetching tag IDs for station " + stationId + ": " + stationTagError.message);
    }

    const tagIds = stationTagData.map(({ tag_id }) => tag_id);
    ...
    if (tagNamesError) {
        throw new Error("Error fetching tag names for station " + stationId + ": " + tagNamesError.message);
    }
