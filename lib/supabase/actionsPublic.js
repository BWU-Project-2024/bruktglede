...
export const getStoreById = async (storeId) => {
    ...
    try {
        ...
        console.error("Error fetching store id data:", error);

        return null;
    }
};

export async function getStoreVisions() {
    try {
        const supabase = createClient();
        const { data: storeVisionData, error: storeVisionError } =
            await supabase.from("StoreVision").select("*");

        if (storeVisionError) {
            throw new Error(storeVisionError.message);
        }

        for (let i = 0; i < storeVisionData.length; i++) {
            const storeId = storeVisionData[i].store_id;
            ...
            storeVisionData[i].store_name = storeData[0]
                ? storeData[0].name
                : "Unknown Store";
        }
...
export async function getWeeklyHighlight() {
    try {
        ...
        for (let i = 0; i < highlightData.length; i++) {
            const postId = highlightData[i].id;
            ...
            highlightData[i].tags = tagNamesData.map(({ name }) => name);
        }
        ...
        for (let i = 0; i < highlightData.length; i++) {
            ...
            highlightData[i].store_name = storeData[0].name;
        }
