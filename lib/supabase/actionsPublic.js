...
export async function fetchAllStationsSortedByStoreId(storeId) {
    const supabase = createClient();
    ...
    try {
        ...
        // If there are stations for the store
        if (stationData.length > 0) {
            const stationsWithTags = await Promise.all(
                stationData.map(async (station) => {
                    // Fetch tags for the station
                    const { data: stationTags, error: stationTagsError } =
                        await supabase
                            .from("Station_Tags")
                            .select("tag_id")
                    ...
                    // Return the station with its tags
                    return {
                        ...station,
                        tags: tagsData.map(({ name }) => name),
                    };
            ...
            return stationsWithTags;
        } else {
            return []; // No stations found for the store
        }
    } catch (error) {
        console.error("Error fetching stations data:", error);
        return null;
...
export async function getTopFourEventsId(storeId) {
    const supabase = createClient();
...
export async function getHighlights() {
    ...
    try {
        ...
        // Fetch store name for each highlight
        for (let i = 0; i < highlightData.length; i++) {
            const storeId = highlightData[i].store_id;
            ...
            // Add store name to the current highlight
            highlightData[i].store_name = storeName[0].name;
        }
...
export async function getHighlightsByStore(storeId) {
    const supabase = createClient();;
