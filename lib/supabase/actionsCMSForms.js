...
export const newArticle = async (formData) => {
    ...
    console.log("fileinput db", fileInput);
    ...
    const folerName = accountData[0].store_id;
    //* Insert image to supabase storage (each user has its own folder)
    const { data: imgData, error: imgError } = await supabase.storage
        .from("images")
        .upload(`${folerName}/${fileInput}`, fileInput);
