...
// Create new article
export const newArticle = async (formData, selectedFile) => {
    ...
    console.log("accountData", accountData[0].store_id);

    const folderName = accountData[0].store_id;
    //* Insert image to supabase storage (each store has its own folder)
    const { data: imgData, error: imgError } = await supabase.storage
        .from("images")
        .upload(`${folderName}/${selectedFile.name}`, selectedFile)
    ...
    console.log(imgData);
    //* Get img url from supabase storage
    const { data: urlData, error: urlError } = supabase.storage
        .from("images")
        .getPublicUrl(imgData)
...
export const insertStoreData = async (formData, openingHoursData) => {
    ...
    const { data: storeData, error: storeDataError } = await supabase
        ...
        .update({
            name: butikknavn,
            description: beskrivelse, //gikk ikke igjennom
            address: adresse, // gikk ikke igjennom
            phone: tlfnummer,
            contactEmail: epost,
            img: headerbilde,
        })
...
// Insert data into store
export const insertStoreVisionData = async (formData, openingHoursData) => {
